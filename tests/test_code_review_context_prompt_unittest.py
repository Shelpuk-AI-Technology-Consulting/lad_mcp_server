...
class TestCodeReviewContextPrompt(unittest.TestCase):
    def test_context_is_included_in_user_prompt(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            asyncio.run(
                service.code_review(
                    code=None,
                    paths=[str(repo / "a.txt")],
                    context="Please focus on correctness and boundary conditions.",
                )
            )

        joined = "\n".join(capture.user_messages)
        self.assertIn("Please focus on correctness and boundary conditions.", joined)
