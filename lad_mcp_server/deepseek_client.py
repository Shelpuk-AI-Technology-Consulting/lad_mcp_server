...
_DEEPSEEK_PREFIX_RE = re.compile(r"^deepseek/", re.IGNORECASE)
...
def is_deepseek_model(model: str) -> bool:
    if not isinstance(model, str):
        return False
    return _DEEPSEEK_PREFIX_RE.match(model.strip()) is not None


def normalize_deepseek_model_name(model: str) -> str:
    if not isinstance(model, str):
        return model
    normalized = _DEEPSEEK_PREFIX_RE.sub("", model.strip(), count=1)
        ...
        def _do_request() -> dict[str, Any]:
            try:
                ...
                raise DeepSeekClientError(
                    f"DeepSeek endpoint HTTP {getattr(exc, 'code', 'error')}: {body_text[:300]}"
                ) from exc
            except Exception as exc:
                raise DeepSeekClientError(f"DeepSeek endpoint request failed: {exc}") from exc
            try:
                parsed = json.loads(raw)
            except json.JSONDecodeError as exc:
                raise DeepSeekClientError("DeepSeek endpoint response was not valid JSON") from exc
            if not isinstance(parsed, dict):
                raise DeepSeekClientError("DeepSeek endpoint response JSON was not an object")
            if "error" in parsed:
                raise DeepSeekClientError(f"DeepSeek endpoint error: {parsed.get('error')}")
            return parsed
    ...
    async def chat_completion(
        ...
        *,
        ...
        async with self._get_semaphore():
            ...
            # DeepSeek requires reasoning_content in assistant messages during tool-call rounds.
            # Do NOT strip it (same as Z.AI Preserved Thinking).
            try:
                ...
                raise DeepSeekClientError(f"DeepSeek endpoint request timed out after {timeout_seconds}s") from exc
            except Exception as exc:
                raise DeepSeekClientError(f"DeepSeek endpoint request failed: {exc}") from exc
